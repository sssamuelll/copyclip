...
type Props = {
  ...
  fileLine: string
  onStepThrough: () => void
  /** Optional — omit when the slot is owned by another widget so pressing × cannot kill a foreign playground. */
  onClose?: () => void
  citation?: Citation
  breadcrumb?: string
...
export function IdleInvitation({ funcName, fileLine, onStepThrough, onClose, citation, breadcrumb, onOpenCitation, lang }: Props) {
  return (
    <div className="widget stepper-widget">
      ...
      <div style={{ ...s('display:flex;flex-direction:column;'), height: 430 }}>
        ...
        {breadcrumb || citation ? (
          <div className="playground-breadcrumb-row" style={s('display:flex;align-items:center;gap:8px;padding:0 16px 8px;flex-wrap:wrap;')}>
