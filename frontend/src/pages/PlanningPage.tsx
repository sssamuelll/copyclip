...
export function PlanningPage() {
  ...
  const renderColumn = (col: Column, label: string, emptyLabel: string) => {
    ...
    return (
      <div className="kanban-column" style={{ display: 'flex', flexDirection: 'column', gap: 16, minWidth: 300, background: 'rgba(255,255,255,0.01)', borderRadius: 8, padding: 12, border: '1px solid rgba(255,255,255,0.03)' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          {colTasks.map(task => (
            <div key={task.id} className="panel" style={{ padding: 16, cursor: 'pointer', transition: 'transform 0.1s ease', position: 'relative', overflow: 'hidden' }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 8 }}>
                <span className="muted" style={{ fontSize: 9 }}>{task.type === 'agent' ? 'Agent proposal' : 'Human signal'}</span>
          ...
          {colTasks.length === 0 && <div className="muted" style={{ textAlign: 'center', padding: 40, fontSize: 11, border: '1px dashed #222', borderRadius: 6 }}>{emptyLabel}</div>}
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="page-container">
      <header style={{ marginBottom: 32 }}>
        <div className="muted" style={{ fontSize: 10, letterSpacing: 2 }}>// intent_field</div>
        <h1 style={{ margin: '8px 0 0 0' }}>Intent Field</h1>
        <div className="muted" style={{ marginTop: 8, maxWidth: 720, fontSize: 13 }}>
          Shape the future state of the system. The Intent Field gathers emerging direction, active alignment work, and architectural will already anchored in code.
        </div>
      </header>

      {loading ? (
        <div className="muted">Synchronizing the field…</div>
      ) : (
        <div className="kanban-board" style={{ display: 'flex', gap: 24, overflowX: 'auto', paddingBottom: 24 }}>
          {renderColumn('intentions', 'EMERGING_INTENT', 'No new intention is gathering mass here yet.')}
          {renderColumn('aligning', 'ALIGNMENT_IN_MOTION', 'This region of the field is currently quiet.')}
          {renderColumn('anchored', 'ANCHORED_DIRECTION', 'No decision has been anchored into the system here yet.')}
        </div>
      )}
    </div>
  )
