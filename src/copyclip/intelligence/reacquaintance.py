...
from .db import connect, init_schema, get_reentry_baseline, get_active_decisions
...
def build_reacquaintance_briefing(
    ...
    for r in commit_rows:
        ...
        if baseline_dt and commit_dt and commit_dt < baseline_dt:
            continue
        related_targets = sorted({fc[0] for fc in file_change_rows if fc[0]})[:20]
        candidate_target = None
        best_tuple = (-1, -1)
        for target in related_targets:
            c = churn_by_file.get(target, 0)
            risk_score = max((x["score"] for x in risk_by_area.get(target, [])), default=0)
            if (c, risk_score) > best_tuple:
                best_tuple = (c, risk_score)
        ...
        evidence = [f"commit:{r[0]}"]
        _append_evidence(evidence_index, _evidence_item(f"commit:{r[0]}", "commit", r[3], r[0]))
        if candidate_target:
