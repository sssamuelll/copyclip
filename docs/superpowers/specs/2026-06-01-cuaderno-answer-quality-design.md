...
- **Guaranteed (deterministic, Phase 1):** the user's answer language is enforced by a deterministic mismatch check that drives the one retry (es/en detection; `unknown` is permissive, so very short/ambiguous questions fall back to the prompt instruction); a code-comprehension answer is never sealed as a confident `answer` when it made **zero content-bearing reads** OR when it **cites only unread paths** (fabricated grounding); the verdict is persisted on every terminal path; no terminal path archives a non-answer as an answer.
- **Hoped (best-effort via judge + prompt, explicitly NOT guaranteed):** citation *relevance*; answer *responsiveness* (how vs. what); **and the residual ungrounded case Phase 1 cannot catch** — a code answer that does ONE real but irrelevant read and emits *no* citations satisfies the read-count gate and carries no fabricated citation to flag, so it seals `answer`. The Phase-2 judge closes this. This honesty is itself part of the design: the gap between the checked term ("a content-bearing read happened / no fabricated citation") and the named property ("grounded") is acknowledged here, not hidden — `answer` in Phase 1 means *"the tutor consulted the code and did not cite anything it failed to read,"* which is weaker than *"every claim follows from the evidence."*

## 11. Suggested implementation phases

The spec is one coherent feature but can ship incrementally:

- **Phase 1 (deterministic core, no judge):** `Frame.status` + persistence + all terminal paths + migration default; the cheap layer (content-bearing read ledger, language detection, citation-vs-ledger check); the closing-round decoupling + grounding-retry-once; honest UI rendering; prompt fixes (language mirroring, ground-before-answer, answer-the-question-asked). Without the judge, Phase 1 seals `ungrounded` deterministically in two cases: **zero** content-bearing reads, OR an answer whose citations point **only** to paths never read this turn (fabricated grounding). It also fires the one retry on a detected language mismatch. **`insufficient_evidence` (World A — *consulted, genuinely empty*) is NOT produced in Phase 1** (the plan and shipped code narrow this): distinguishing "the project is genuinely empty" from "the tutor didn't look well" requires the semantic judge, so World A is deferred to Phase 2. Phase 1 therefore ships `ungrounded` (World B) + `answer`; the two-world split becomes real only with the judge. UI-chrome i18n ("you asked" / "go deeper") is also deferred to Phase 2 — in Phase 1 the answer's own blocks, kickers and follow-up labels mirror the language because the model produces them, but the fixed chrome stays English.
...
## 15. Post-implementation review (capable-roster, 2026-06-02)

After Phase 1 shipped (PR #125), a five-lens panel (Halberg runtime, Serrano spec-risk, Voronov + Null Vale ontology/honesty, Vex Rune deletion) re-audited the **code**. Fixes applied on the branch:

- **Discarded-block leak (Halberg, correctness):** the grounding retry's `emitted.clear()` cleared only the compositor's block list, not the HTTP wrapper's separate buffer — a disconnect during the retry round persisted the discarded `ungrounded` block as `partial`. Fixed with a `reset` protocol event (compositor → ask_stream clears + forwards → frontend drops its provisional render).
- **`answer` = "touched the filesystem" (Voronov/Null Vale, integrity):** the predicate was only `content_bearing_count == 0`, so a `list_dir` + a fabricated citation sealed `answer`. The previously-collected-but-unused `read_paths` is now consulted: a code answer citing **only** unread paths seals `ungrounded`. Residual (one irrelevant read + no citations) is named in §10 as a Phase-2 judge gap.
- **Language no-op (Serrano):** `language_mismatch` was computed and discarded; the one retry now fires on it (composed grounding/language directive), making the §10 language guarantee real.
- **Honest banners (Voronov/Null Vale, UX):** the `ungrounded` banner no longer asserts World B ("the tutor was lazy"); it names both worlds. `legacy` frames now carry a subtle "predates grounding checks" note so historical unchecked answers — including the original incident answer — are distinguishable from verified ones.

Accepted/deferred to Phase 2 (judge): the bare-no-citation laundering residual (§10); `insufficient_evidence` / World A production (§11); responsiveness and citation *relevance* (§4.3); UI-chrome i18n (§11). Vex Rune's cleanups (the `_sealed_frame` helper, `QualityVerdict.reason`, dead guards) were noted but **not** applied — out of the review's fix scope.
