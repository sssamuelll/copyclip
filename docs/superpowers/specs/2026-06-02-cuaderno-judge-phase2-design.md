...
- **Prompt injection (HIGH).** The judged answer is fenced as untrusted `DATA` with a `JUDGE_PROMPT` instruction to never follow instructions inside it. A re-audit caught that a static fence is theater (an answer can spell the closing token), so the fence uses an **unpredictable per-call nonce** the answer — authored before the call — cannot spell.
...
- **Frontend (#11/#12).** Follow-ups are suppressed on non-`answer` dispositions; a pre-Phase-2 `answer` (no `verdict`) carries the same "predates grounding checks" provenance note as `legacy`. The re-audit also caught that a **judge-outage** answer (`source: unjudged`) was sealed `answer` but, having a non-null verdict, showed no provenance note — so the surface presented an unchecked answer as judged. Fixed: the UI now shows *"the reviewer was unavailable — this answer was not checked for responsiveness"* when `verdict.source === 'unjudged'`.

Accepted residuals: the persisted `verdict` is still unread by the UI *contents* (the UI reads `status` and `verdict.source`/presence; full verdict surfacing is Phase 3, now with honest values); a responsiveness-retry round that returns cheap-ungrounded seals `ungrounded` (honest) rather than re-judging; the `world`-unknown disposition is recoverable-by-design; the judge's `reason` is persisted verbatim (a Phase-3 render-surface to keep in mind). Vex's cleanups applied (reused `_answer_text`, simplified `_extract_json`, keyword `_ok_verdict`); the server config read keeps its guard.
