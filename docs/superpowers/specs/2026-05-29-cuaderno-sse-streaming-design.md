...
| `api/cuaderno.ts:3-44` | Add `askStream(question, sessionId, { onEvent, signal }): Promise<void>` — `fetch` POST (keeps the JSON body) + `response.body.getReader()` + `TextDecoder`, splitting on `\n\n` SSE record boundaries, parsing each `data:` line to a typed `CuadernoStreamEvent`, dispatching via `onEvent`, resolving when the stream closes. Accept an `AbortSignal`. Session restore uses `session()`/`getJson` and markers use `patchQuestion`/`patchJson`, all unchanged; the one-shot `ask`/`postJson` become dead once the page switches to `askStream` and are removed in PR4. |
...
- **Zero blocks / budget exhausted**: the compositor yields a terminal `frame` event carrying the `_fallback_frame` (`compositor.py:68-72, 99`). This is a graceful fallback, not a stream error — surfacing it as `frame` (not `error`) keeps live-vs-restore parity, since the non-streaming `compose_frame` wrapper returns the same fallback Frame. The `error` event is reserved for genuine stream-level failures (LLM API exception mid-stream, client disconnect).
